    ...
    <dropdown align="right" width="400px" v-cloak>
        <template v-slot:trigger>
            <li class="flex items-center rounded-full bg-transparent px-1 py-1 hover:bg-gray-200 hover:text-black cursor-pointer">
                <img src="{{ current_user()->avatar }}" alt="" class="rounded-full mr-2" width="30" height="30">

                <div class="flex flex-col mr-5 text-left">
                    <span class="text-sm font-bold"> {{ Str::limit(current_user()->name,12) }}</span>
                    <span
                        class="text-xs text-gray-700 font-semibold">{{ Str::limit('@'. current_user()->username,12) }}</span>
                </div>
        ...
        <div class="rounded shadow w-full">
            <div class="px-3 py-3 flex items-center pb-4 border-b border-gray-400">
                <img src="{{ current_user()->avatar }}" alt="" class="rounded-full mr-2" width="40" height="40">

                <div class="flex flex-col mr-5 items-start">
                    <span
                        class="font-bold text-left text-sm whitespace-no-wrap overflow-hidden"> {{ current_user()->name }}</span>
            ...
            <div class="px-3 py-3 flex items-center hover:bg-gray-300">
                <form method="POST" action="/logout">
                    ...
                    <button class="px-3 w-full py-3 flex items-center hover:bg-gray-300 text-sm text-red-600"
                            type="Submit">
