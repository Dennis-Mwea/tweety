<x-app>
    @if ($unreadNotifications->count())
        ...
        @foreach ($unreadNotifications as $date => $unread)
            <div class="mb-6 text-right">
                <span
                    class="px-5 py-3 rounded-xl border border-blue-500 text-blue-700 shadow-lg font-semibold text-lg">
                    {{ $date }}</span>
            </div>

            @foreach($unread as $unreadNotification)
                <div class="flex bg-white shadow p-4 rounded-xl mb-4 border border-gray-300">
                    <div class="flex w-full">
                        ...
                        <div class="flex-1 text-right tracking-tight">
                            {{--                        <h3 class="font-semibold broder border-r-4 {{ get_notification_color($unreadNotification->type) }} p-2">--}}
                            {{--                            {{ $unreadNotification->created_at->format('M d') }} </h3>--}}
                            <span
                                class="text-gray-700 text-sm font-bold">
                            {{ $unreadNotification->created_at->diffForHumans() }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    @if ($readNotifications->count())
        ...
        @foreach ($readNotifications as $date => $read)
            <div class="mb-6 text-right">
                <span
                    class="px-5 py-3 rounded-xl border border-blue-500 text-blue-700 shadow-lg font-semibold text-lg">
                    {{ $date }}</span>
            ...
            @foreach ($read as $readNotification)
                <div class="flex bg-white shadow p-4 rounded-xl mb-4 border border-gray-300">
                    <div class="flex w-full">
                        ...
                        <div class="flex-1 text-right tracking-tight">
                            ...
                            <span
                                class="text-gray-700 text-sm font-bold">{{ $readNotification->created_at->diffForHumans() }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
