<div class="p-4 {{ $loop->last ? '' :  'border-b border-gray-400'}}">
    ...
    <div class="flex">
        ...
        <div class="flex-1">
            ...
            @if($reply->image !== null)
                <div class="mb-3">
                    <img src="{{ asset($reply->image) }}" alt="tweet-image"
                         class="rounded-lg mb-1 h-64 w-full object-cover" width="50" height="50">
                </div>
            @endif

            <div class="flex mb-4 py-3">
                <button
                    @click.prevent="$modal.show('add-reply',{'tweetID':{{$reply->tweet->id }},'parentID':{{$reply->id }}})">
